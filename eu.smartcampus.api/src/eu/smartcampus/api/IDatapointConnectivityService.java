...
public interface IDatapointConnectivityService {
	...
	/**
	 * The type of error found in read of write operations.
	 */
	enum ErrorType {
		DATAPOINT_NOT_FOUND, DATAPOINT_CONNECTION_ERROR, DATAPOINT_NOT_RESPONDING, DATAPOINT_VANISHED, DEVICE_BUSY, GATEWAY_CONNECTION_ERROR, GATEWAY_NOT_FOUND, GATEWAY_NOT_RESPONDING, GATEWAY_BUSY
	}
	...
	interface ReadCallback {
		 ...
		 *            the request id
		 */
		void onReadCompleted(DatapointAddress address, Reading[] readings,
				int requestId);
	}
	...
	/**
	 * Level of confirmation of datapoint write operations.
	 */
	 ...
	 * 
	 * @param address
	 *            the set of datapoints, belonging to the sensor, for which we
	 ...
	 * @param clientKey
	 *            the identifier of the client making the request * @return the
	 *            id of the write request used to by the client to identify the
	 ...
	 * <p>
	 * Implementations of this method may query the meter or return cached
	 * values. Since meters are devices with limited resources, specifying
	 * <tt>start</tt> to far into the past may result in only a subset of the
	 * values.
	 * 
	 * @param address
	 *            the absolute address of the datapoint to be read
	 ...
	 *            equal to start.
	 * @param clientKey
	 *            the identifier of the client making the request
	 * @return the id of the read request used to by the client to identify the
	 *         acknowledge of this request
	 * 
	 * @return <ol>
	 *         <li>an empty array, if <tt>start &gt; finish</tt></li><li>a
	 *         single reading <i>r</i>, if available, such that <tt><i>r.ts</i>
	 *         == start == finish</tt>, if <tt>start == finish</tt></li> <li>all
	 *         readings <i>r</i>, if available, such that <tt>start
	 *         &le;<i>r.ts</i> &le; finish</tt>
	 *         </ol>
	 */
	int requestDatapointWindowRead(DatapointAddress address, Timestamp start,
			Timestamp finish, ReadCallback readCallback);
	 ...
	 *         acknowledge of this request
	 */
	int requestDatapointWrite(DatapointAddress address, Value[] values,
