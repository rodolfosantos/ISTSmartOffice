...
import ist.smartoffice.datapointconnectivity.DatapointComposed;
...
import ist.smartoffice.datapointconnectivity.DatapointSimple;
...
public class KNXServiceConfig {
	 ...
	 * @return the map
	 */
	public static Map<DatapointAddress, IDatapoint> loadDatapointsConfigs() {
		// load datapoins from json file
//		@SuppressWarnings("unchecked")
//		Map<DatapointAddress, IDatapoint> datapointSettings = (Map<DatapointAddress, IDatapoint>) KNXServiceConfig
//				.fromJsonFile(CONFIG_FILENAME,
//						new TypeToken<Map<String, DatapointMetadata>>() {
//						}.getType());
//
//		Map<DatapointAddress, IDatapoint> datapoints = new LinkedHashMap<DatapointAddress, IDatapoint>();
//
//		if (datapointSettings != null) {
//			Iterator<Entry<DatapointAddress, IDatapoint>> it = datapointSettings
//					.entrySet().iterator();
//			while (it.hasNext()) {
//				Map.Entry<DatapointAddress, IDatapoint> entry = (Map.Entry<DatapointAddress, IDatapoint>) it
//						.next();
//				datapoints.put(new DatapointAddress(entry.getKey().getAddress()),
//						entry.getValue());
//			}
//			return datapoints;
//		} else {
...
//		}
	 ...
	 * @return the map
	 */
	private static Map<DatapointAddress, IDatapoint> defaultKNXDatapointsConfig() {

		// knx datapoints
		Map<DatapointAddress, IDatapoint> knxDatapoints = new LinkedHashMap<DatapointAddress, IDatapoint>();
		...
		DatapointAddress d1 = new DatapointAddress("0/4/5");
		IDatapoint dd1 = new DatapointSimple(d1, knxMetadata1.build());
		
		
		MetadataBuilder knxMetadata2 = new DatapointMetadata.MetadataBuilder();
		knxMetadata2.setDescription("EnergyLab Blinds 1");
		knxMetadata2.setAccessType(AccessType.READ_WRITE);
		knxMetadata2.setDatatype(Datatype.PERCENTAGE);
		DatapointAddress d2r = new DatapointAddress("0/2/0");
		DatapointAddress d2w = new DatapointAddress("0/2/3");
		IDatapoint dd2 = new DatapointComposed(d2r, d2w, knxMetadata2.build());
		
		knxDatapoints.put(dd1.getDatapointMainAddress(), dd1);
		knxDatapoints.put(dd2.getDatapointMainAddress(), dd2);
